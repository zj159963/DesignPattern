...
- (void)awakeFromNib {
  ...
  self.totalValue = 0.0;
  self.charge = [CHChargeSetter createChargeWithType:CHChargeTypeOriginal];
}
...
- (IBAction)confirmButtonTapped:(id)sender {
  ...
  [UIView animateWithDuration:0.25 animations:^{
    ...
    self.charge.originalValue = self.totalValue;
    self.totalLabel.text = self.charge.realCharge;
    [self refreshFields];
...
- (void)pickerView:(UIPickerView *)pickerView didSelectRow:(NSInteger)row inComponent:(NSInteger)component {
  self.charge = [self chargeWithRow:row];
  self.charge.originalValue = self.totalValue;
  self.calculationLabel.text = self.charge.description;
  self.view.userInteractionEnabled = NO;
  [UIView animateWithDuration:0.25 animations:^{
...
- (CHBaseCharge *)chargeWithRow:(NSInteger)row
{
  ...
  return [CHChargeSetter createChargeWithType:type];
}
